...
struct NewGamesCarouselView: View {
    ...
    var newGames: [Game] {
        var filtered = games
        filtered = filtered.filter { $0.broken }
                ...
                GeometryReader { geometry in
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: self.spacing) {
                            ForEach(newGames.indices, id: \.self) { index in
                                let game = newGames[index]
                                GameTileView(
                                    game: game,
