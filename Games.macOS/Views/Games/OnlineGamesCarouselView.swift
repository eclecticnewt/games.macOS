...
struct OnlineGamesCarouselView: View {
    ...
    var onlineGames: [Game] {
        var filtered = games.filter { $0.online }
        switch sortOption {
        case "Title":
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            ...
            if !onlineGames.isEmpty {
                GeometryReader { geometry in
                    ScrollView(.horizontal, showsIndicators: false) {
                        HStack(spacing: self.spacing) {
                            ForEach(onlineGames.indices, id: \.self) { index in
                                let game = onlineGames[index]
                                GameTileView(
                                    game: game,
                ...
                Spacer()
                Text("No online games available.")
                    .font(.title2)
