...
struct SearchGamesGridView: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            // Title stays aligned to the left, 20 points from the left edge
            ...
            HStack(alignment: .top, spacing: 10) {
                // Keyboard appears to the left, under the title, only when isOnScreenKeyboard is true
                if isOnScreenKeyboard {
                    VStack {
                        KeyboardGridView(searchText: $searchText)
                ...
                // Game grid is displayed to the right of the keyboard or fills the space if the keyboard isn't showing
                VStack(alignment: .leading, spacing: 10) {
                    if searchText.isEmpty {
                        Spacer()
                            ...
                            ScrollView {
                                LazyVGrid(columns: [GridItem(.adaptive(minimum: tileWidth), spacing: spacing)], spacing: spacing) {
                                    ForEach(filteredGames.indices, id: \.self) { index in
                                        ...
                                        GameTileView(
                                            game: game,
                                            isSelected: selectedGameId == game.id, // Correct binding for selection
                                            onTap: {
                                                handleSelection(for: game) // Ensure correct selection
                                            },
